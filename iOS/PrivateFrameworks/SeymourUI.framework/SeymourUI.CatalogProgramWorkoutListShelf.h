/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/SeymourUI.framework/SeymourUI
 */

@interface SeymourUI.CatalogProgramWorkoutListShelf : _TtCs12_SwiftObject {
    void delegate;
    void downloadSubscriptionCoordinator;
    void eventHub;
    union { in /* Warning: Unrecognized filer type: 'u' using 'void*' */ void*x1; long x2; long x3; }  identifier;
    void lazyLockupFetcher;
    void mediaTagStringBuilder;
    void platform;
    union { in /* Warning: Unrecognized filer type: 'u' using 'void*' */ void*x1; long x2; long x3; }  program;
    void row;
    void storefrontLocalizer;
    void subscriptionCache;
    void subscriptionToken;
}

@end
